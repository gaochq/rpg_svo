...
public:
    ...
    Sophus::SE3                   T_f_w_;                 //!< Transform (f)rame from (w)orld.  world => frame
    ...
    /*
     *  参考论文式(11)中雅克比矩阵的链式展开((11)式下面的式子）
     *  这个地方的求导对应链式中的后两项，也就是投影坐标(uv)对T的求导，即
     *
     *  d(u,v)/dT=d(u,v)/dP' * dP'/dT = d(π(P'))/dP' * d(TP)/dT
     *
     *  其中  |u|   |1 0 0||X'|
     *      Z|v| = |0 1 0||Y'|   ===>
     *       |1|   |0 0 1||Z'|
     *
     *                  |1/Z' 0  -X'/Z'^2|
     *  d(π(P'))/dP'= - |0  1/Z' -Y'/Z'^2|
     *
     *                    |1  0  0   0    Z'  -Y'|
     *  d(TP)/dT=|I -P'^|=|0  1  0  -Z'   0    X'|
     *                    |0  0  1   Y'  -X'  0 |
     *
     *  将上两式相乘得，===>
     *
     *  J = - |1/Z'  0  -X'/Z'^2   -X'Y'/Z'^2   1+X'^2/Z'^2  -Y'/Z'|
     *        | 0  1/Z' -Y'/Z'^2  -1-X'^2/Z'^2   X'Y'/Z'^2    X'/Z'|
     *
     */
